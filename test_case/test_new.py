...
@pytest.mark.sale
@pytest.mark.parametrize("casename1,head,param",datas)
def test_sale(casename1,head,param):
    ...
    resus = resu.text
    logger.info("【%s】开始返回数据==========：%s"%(casename1,resus))
    logger.info("【%s】结束返回数据==========：%s"%(casename1,'end'))
    ...
    logger.info("transactionId======%s,tradeNo=======%s"%(transactionId,tradeNo))
